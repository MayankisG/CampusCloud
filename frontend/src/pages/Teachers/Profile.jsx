...
const TeacherProfileSection = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (profileResponse.data?.teacherImage) {
          try {
            const imgResponse = await axios.get(
              `${API_BASE_URL}/faculty/profile-image/${email}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
            ...
            localStorage.setItem("teacherProfileImage", blobUrl);
          } catch (imgError) {
            console.warn(
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/faculty/upload-image`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleCancelUpload = () => {
    ...
    if (teacherProfile?.teacherImage) {
        ...
        {teacherProfile ? (
          <>
            ...
            <ProfileInfo>
              <ProfileDetail>
                <Label>Name:</Label>
                <Value>{teacherProfile.name || "N/A"}</Value>
              </ProfileDetail>
              <ProfileDetail>
                <Label>Email:</Label>
                <Value>{teacherProfile.email || "N/A"}</Value>
              </ProfileDetail>
              <ProfileDetail>
                <Label>University ID:</Label>
                <Value>{teacherProfile.univId || "N/A"}</Value>
              </ProfileDetail>
              <ProfileDetail>
                <Label>Department:</Label>
                <Value>{teacherProfile.department || "N/A"}</Value>
              </ProfileDetail>
              <ProfileDetail>
                <Label>DOB:</Label>
                <Value>{teacherProfile.dob || "N/A"}</Value>
              </ProfileDetail>

              <ProfileDetail>
                <Label>Contact Number:</Label>
                <Value>{teacherProfile.contactNo || "N/A"}</Value>
              </ProfileDetail>
              <ProfileDetail>
                <Label>Address:</Label>
                <Value>{teacherProfile.address || "N/A"}</Value>
              </ProfileDetail>
            </ProfileInfo>
          </>
        ) : (
